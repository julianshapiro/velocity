...
(function(VelocityStatic) {
    function completeCall(activeCall, isStopped) {
        ...
        for (var i = 0, callLength = call.length; i < callLength; i++) {
            ...
            if (!isStopped && !opts.loop) {
                if (opts.display === "none") {
                    VelocityStatic.CSS.setPropertyValue(element, "display", opts.display);
                }
                if (opts.visibility === "hidden") {
                    VelocityStatic.CSS.setPropertyValue(element, "visibility", opts.visibility);
                }
            }
            ...
            if ((opts.loop !== true || isStopped) && ($.queue(element)[1] === undefined || !/\.velocityQueueEntryFlag/i.test($.queue(element)[1]))) {
                if (data) {
                    ...
                    $.each(VelocityStatic.CSS.Lists.transforms3D, function(i, transformName) {
        ...
        if (VelocityStatic.State.first === activeCall) {
            VelocityStatic.State.first = activeCall.next;
        } else if (activeCall.prev) {
            activeCall.prev.next = activeCall.next;
        }
        if (VelocityStatic.State.last === activeCall) {
            VelocityStatic.State.last = activeCall.prev;
        ...
        if (!VelocityStatic.State.first) {
            ...
            VelocityStatic.State.cache = undefined;
        } else {
            for (var key in activeCall) {
                activeCall[key] = undefined;
            }
            activeCall.next = VelocityStatic.State.cache;
            VelocityStatic.State.cache = activeCall;
        }
    }
...
(function(VelocityStatic) {
    VelocityStatic.animate = Velocity;
    VelocityStatic.Utilities = $;
...
(function(VelocityStatic) {
    var ticker, rAFShim = ticker = function() {
        var timeLast = 0;
    ...
    function tick(timestamp) {
        if (timestamp) {
            var timeCurrent = timestamp && timestamp !== true ? timestamp : performance.now(), activeCall = VelocityStatic.State.first, nextCall;
            for (;activeCall; activeCall = nextCall) {
                ...
                var call = activeCall.call, opts = activeCall.options, tweenDummyValue = null, millisecondsEllapsed = activeCall.ellapsedTime = timeCurrent - timeStart, percentComplete = Math.min(millisecondsEllapsed / opts.duration, 1);
                for (var j = 0, callLength = call.length; j < callLength; j++) {
                    ...
                    if (opts.display !== undefined && opts.display !== null && opts.display !== "none") {
                        if (opts.display === "flex") {
                            ...
                            $.each(flexValues, function(i, flexValue) {
                                VelocityStatic.CSS.setPropertyValue(element, "display", flexValue);
                            });
                        }
                        VelocityStatic.CSS.setPropertyValue(element, "display", opts.display);
                    }
                    ...
                    for (var property in tweensContainer) {
                        if (tweensContainer.hasOwnProperty(property) && property !== "element") {
                            var tween = tweensContainer[property], currentValue, easing = isString(tween.easing) ? VelocityStatic.Easings[tween.easing] : tween.easing;
                            ...
                            if (property === "tween") {
                                ...
                                var adjustedSetData = VelocityStatic.CSS.setPropertyValue(element, property, tween.currentValue + (IE < 9 && parseFloat(currentValue) === 0 ? "" : tween.unitType), tween.rootPropertyValue, tween.scrollData);
                                if (VelocityStatic.CSS.Hooks.registered[property]) {
                                    if (VelocityStatic.CSS.Normalizations.registered[hookRoot]) {
                                        Data(element).rootPropertyValueCache[hookRoot] = VelocityStatic.CSS.Normalizations.registered[hookRoot]("extract", null, adjustedSetData[1]);
                                    } else {
...
function Velocity() {
    ...
    function processElement(element, elementArrayIndex) {
        var opts = _assign({}, VelocityStatic.defaults, options), tweensContainer = {}, elementUnitConversionData;
        if (Data(element) === undefined) {
            VelocityStatic.init(element);
        }
        ...
        if (VelocityStatic.mock !== false) {
            if (VelocityStatic.mock === true) {
                ...
                opts.delay = parseFloat(opts.delay) * parseFloat(VelocityStatic.mock) || 1;
            }
        }
        ...
        function buildQueue() {
            ...
            if (action === "scroll") {
                ...
                if (opts.container) {
                    ...
                    scrollPositionCurrent = VelocityStatic.State.scrollAnchor[VelocityStatic.State["scrollProperty" + scrollDirection]];
                    scrollPositionCurrentAlternate = VelocityStatic.State.scrollAnchor[VelocityStatic.State["scrollProperty" + (scrollDirection === "Left" ? "Top" : "Left")]];
                ...
                var fixPropertyValue = function(property, valueData) {
                    ...
                    var calculateUnitRatios = function() {
                        var sameRatioIndicators = {
                            ...
                            position: VelocityStatic.CSS.getPropertyValue(element, "position"),
                            fontSize: VelocityStatic.CSS.getPropertyValue(element, "fontSize")
                        ...
                        if (!sameEmRatio || !samePercentRatio) {
                            ...
                            unitRatios.percentToPxWidth = callUnitConversionData.lastPercentToPxWidth = (parseFloat(VelocityStatic.CSS.getPropertyValue(dummy, "width", null, true)) || 1) / measurement;
                            unitRatios.percentToPxHeight = callUnitConversionData.lastPercentToPxHeight = (parseFloat(VelocityStatic.CSS.getPropertyValue(dummy, "height", null, true)) || 1) / measurement;
                            unitRatios.emToPx = callUnitConversionData.lastEmToPx = (parseFloat(VelocityStatic.CSS.getPropertyValue(dummy, "paddingLeft")) || 1) / measurement;
                        ...
                        if (callUnitConversionData.remToPx === null) {
                            callUnitConversionData.remToPx = parseFloat(VelocityStatic.CSS.getPropertyValue(document.body, "fontSize")) || 16;
                        }
                ...
                for (var property in propertiesMap) {
                    ...
                    if (_inArray(VelocityStatic.CSS.Lists.colors, propertyName)) {
                        ...
                        if (VelocityStatic.CSS.RegEx.isHex.test(endValue)) {
                            var colorComponents = [ "Red", "Green", "Blue" ], endValueRGB = VelocityStatic.CSS.Values.hexToRgb(endValue), startValueRGB = startValue ? VelocityStatic.CSS.Values.hexToRgb(startValue) : undefined;
            ...
            if (tweensContainer.element) {
                ...
                if (elementsIndex === elementsLength - 1) {
                    var last = VelocityStatic.State.last, next;
...
var global = this, $ = global.fn && global.fn.jquery ? global : window.jQuery;
