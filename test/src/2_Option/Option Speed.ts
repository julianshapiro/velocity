...
import {asyncTests, defaultProperties, getNow, getTarget} from "../app";
import "./_module";
import {Velocity, VelocityExtended} from "../../../index.d";
...
QUnit.test("Speed", function(assert) {
	...
	asyncTests(assert, 1, function(done) {
		Velocity(getTarget(), defaultProperties, {
			duration: duration,
			begin: function(elements: ExtendedVelocityExtended) {
				elements.__start = getNow();
			},
			complete: function(elements: ExtendedVelocityExtended) {
	...
	asyncTests(assert, 1, function(done) {
		Velocity(getTarget(), defaultProperties, {
			duration: duration,
			speed: 0,
			progress: function(elements: ExtendedVelocityExtended, percentComplete) {
				if (!elements.__count) {
