...
QUnit.todo("Arguments", function(assert) {
	...
	Velocity($target3, defaultProperties, testDuration).catch().then(function() {
		assert.equal(Data($target4).lastAnimationList[""].duration, testDuration, "Overload variation #2: Velocity(element, properties, duration)");
	}).then(done);

	var $target4 = getTarget();
	Velocity($target4, defaultProperties, testEasing).catch().then(function() {
		assert.equal(Data($target4).lastAnimationList[""].easing, testEasing, "Overload variation #3: Velocity(element, properties, easing)");
	}).then(done);
	...
	assert.equal(Data($target6).opts.easing(0.2), 0.0816598562658975, "Overload variation #5b.");
	...
	assert.equal(Data($target8).opts.easing, testEasing, "Overload variation #7b.");
