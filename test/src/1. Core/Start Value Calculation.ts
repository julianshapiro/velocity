...
QUnit.todo("Start Value Calculation", function(assert) {
	...
	assert.equal(Data($target2).style.width.startValue, parseFloat(defaultStyles.width as any), "Defined start value #1 was calculated.");
	assert.equal(Data($target2).style.opacity.startValue, parseFloat(defaultStyles.opacity as any), "Defined start value #2 was calculated.");
	assert.equal(Data($target2).style.colorGreen.startValue, parseFloat(defaultStyles.colorGreen as any), "Defined hooked start value was calculated.");
	...
	assert.equal(Math.floor(Data($target4).style.marginBottom.startValue), parseFloat(testStartValues.marginBottom) / 100 * parseFloat($target4.parentElement.offsetWidth as any), "Property converted to px.");
	...
	assert.equal(Math.round(Data($target5).style.left.startValue), Math.round(parseFloat(testPropertiesTRBL.left) + parseFloat(Velocity.CSS.getPropertyValue(document.body, "marginLeft") as any)), "TRBL value was deferred to jQuery.");
});
