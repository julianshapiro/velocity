...
// Typedefs
import {
	AnimationCall, Properties, Sequence, SequenceList,
	TweenStep, VelocityEasingType, VelocityPromise, VelocityResult,
} from "../../velocity";
 ...
 *
 */
function tweenAction(args: any[], elements: VelocityResult, _promiseHandler: VelocityPromise, _action: string): any {
...
Velocity(<element>, "tween", percentComplete, {property: end | [end, <easing>, <start>], ...}, <easing>) => {property: value, ...}`);
	...
	const fakeAnimation = {
		...
		tweens: null as any as { [property: string]: Sequence },
	...
	let easing: VelocityEasingType = args[2];
	let count = 0;
	...
	for (const property in fakeAnimation.tweens) {
		...
		if (pattern) {
			...
			const tweenEasing = tweenTo.easing || linearEasing;
