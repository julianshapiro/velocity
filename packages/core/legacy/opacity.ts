...
namespace VelocityStatic.CSS {
	if (IE <= 8) {
		 ...
		 */
		function opacity(element: KnownElements): string;
		function opacity(element: KnownElements, propertyValue: string): boolean;
		function opacity(element: KnownElements, propertyValue?: string): string | boolean {
