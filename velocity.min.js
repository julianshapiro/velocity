...
/**
   ...
   */
function C(e,t){return o(e)?e:l(e)?h[e.toLowerCase()]||parseFloat(e.replace("ms","").replace("s","000")):null==t?void 0:C(t)}
/**
...
/**
   ...
   */function H(e){var t=C(e);if(!isNaN(t))return t;null!=e&&console.error("VelocityJS: Trying to set 'delay' to an invalid value:",e)}
/**
   * Validate a <code>duration</code> option.
   */function R(e,t){var n=C(e);if(!isNaN(n)&&n>=0)return n;null==e||t||console.error("VelocityJS: Trying to set 'duration' to an invalid value:",e)}
/**
...
// Steps
return i=e[0],I[i]||(I[i]=function(e,t,n){return 0===e?t:1===e?n:t+Math.round(e*i)*(1/i)*(n-t)});if(2===e.length)
...
// Constants
var Ne=/#([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})/gi,Le=/#([a-f\d])([a-f\d])([a-f\d])/gi,Je=/(rgba?\(\s*)?(\b[a-z]+\b)/g,Ie=/rgb(a?)\(([^\)]+)\)/gi,Ce=/\s+/g,je={};
/**
...
/**
   ...
   */function Fe(e){return e.replace(Ne,Pe).replace(Le,function(e,t,n,r){return Pe(0,t+t,n+n,r+r)}).replace(Je,function(e,t,n){return je[n]?(t||"rgba(")+je[n]+(t?"":",1)"):e}).replace(Ie,function(e,t,n){return"rgba("+n.replace(Ce,"")+(t?"":",1")+")"})}
// Project
/**
...
return qe(e,"display","auto"),i=He(e,t,!0)+"px",qe(e,"display","none"),String(i)}
...
(i=r[t])||(i=e.style[t]),"auto"===i)switch(t){case"width":case"height":i=He(e,t,!0)+"px";break;case"top":case"left":case"right":case"bottom":var o=We(e,"position");if("fixed"===o||"absolute"===o){
...
for(var A=u.length-1,N=0;N<y.length;N++){var L=y[N],J=L[0],I="*"===J||"/"===J,C=I||"+"===J||"-"===J;I&&(
// TODO: Not sure this should be done automatically!
u[A]+="(",f(")")),N&&f(" "+(C?J:"+")+" "),u.push(!1);for(var j=0;j<n;j++){var P=g[j],F=P[1]===L?P[0]:3===P.length?s[j-1][s[j-1].length-1]:I?1:0;s[j].push(F)}f(C?L.substring(1):L)}f(")")}}
...
var V=T[q],A=V.sequence,N=A.pattern,L="",J=0;if(N){for(var I=(V.easing||k)(O,0,1,q),C=0,j=0;j<A.length-1;j++)A[j].percent<I&&(C=j);for(var P=A[C],F=A[C+1]||P,H=(O-P.percent)/(F.percent-P.percent),R=F.easing||S;J<N.length;J++){var W=P[J];if(null==W)L+=N[J];else{var B=F[J];if(W===B)L+=W;else{
...
// For every element, iterate through each property.
var k=c.tweens[x],_=k.sequence,E=_.pattern,O="",T=0;if(h++,E){for(var M=(k.easing||b)(u,0,1,x),q=0,V=0;V<_.length-1;V++)_[V].percent<M&&(q=V);for(var A=_[q],N=_[q+1]||A,L=(u-A.percent)/(N.percent-A.percent),J=N.easing||S;T<E.length;T++){var I=A[T];if(null==I)O+=E[T];else{var C=N[T];if(I===C)O+=I;else{
// All easings must deal with numbers except for our internal ones.
var j=J(L,I,C,x);O+=!0===E[T]?Math.round(j):j}}}f[x]=O}}if(p&&1===h)for(var P in f)if(f.hasOwnProperty(P))return f[P];return f}],!0);
// Project
/**
...
/**
   ...
   */var At=/^(webkit|moz|ms|o)[A-Z]/,Nt=be.prefixElement;for(var Lt in Nt.style)if(At.test(Lt)){var Jt=Lt.replace(/^[a-z]+([A-Z])/,function(e,t){return t.toLowerCase()}),It=Mt.test(Jt)?"px":void 0;Oe(["Element",Jt,qt(Lt,Jt),It])}else if(!Te([Element,Lt])){var Ct=Mt.test(Lt)?"px":void 0;Oe(["Element",Lt,Vt(Lt),Ct])}
// Project
/**
...
// starts.
var L=b.replace(/\..*$/,""),J=g[L];if(J){var I=J(A,w,N,b);return void 0!==I?I:w||k}if(!M)return void console.error("VelocityJS: First argument ("+b+") was not a property map, a known action, or a registered redirect. Aborting.")}var C=void 0;if(a(b)||O||M){
/**
...
k&&(f(j,"_promise",k),f(j,"_rejecter",E),f(j,"_resolver",_)),f(j,"_ready",0),f(j,"_started",0),f(j,"_completed",0),f(j,"_total",0),a(S)){var Z=R(S.duration);C=void 0!==Z,j.duration=d(Z,v.duration),j.delay=d(H(S.delay),v.delay),
...
var te=R(p[h],!0),ne=0;if(C=void 0!==te,void 0!==te&&(ne++,j.duration=te),!r(p[h+ne])){
// Despite coming before Complete, we can't pass a fn easing
var re=W(p[h+ne],d(j&&R(j.duration),v.duration),!0);void 0!==re&&(ne++,j.easing=re)}var ie=F(p[h+ne],!0);void 0!==ie&&(j.complete=ie),j.loop=v.loop,j.repeat=j.repeatAgain=v.repeat}if(O&&!1===j.queue)throw new Error("VelocityJS: Cannot reverse a queue:false animation.");M&&!C&&M.duration&&(j.duration=M.duration);
