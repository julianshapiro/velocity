...
import {isFunction, isPlainObject, isString} from "../types";
 ...
 */
export function defaultAction(args?: any[], elements?: HTMLorSVGElement[] | VelocityResult, promiseHandler?: VelocityPromise, action?: string): void {
  // TODO: default is wrong, should be runSequence based, and needs all arguments
  if (isString(action) && Redirects[action]) {
    ...
    elements.forEach(function (element, elementIndex) {
      ...
      Redirects[action].call(element, element, opts, elementIndex, elements.length, elements, promiseHandler && promiseHandler._resolver);
    });
