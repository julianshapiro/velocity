 ...
 */
export function styleAction(args?: any[], elements?: VelocityResult, promiseHandler?: VelocityPromise, action?: string): any {
  ...
  if (value === undefined && !isPlainObject(property)) {
    ...
    if (elements.length === 1) {
      return getPropertyValue(elements[0], property);
    }
    const result = [];

    for (let i = 0; i < elements.length; i++) {
      result.push(getPropertyValue(elements[i], property));
    }
    return result;
  }
  // SET
  ...
  if (isPlainObject(property)) {
    for (const propertyName in property) {
      for (let i = 0; i < elements.length; i++) {
        ...
        if (isString(value) || isNumber(value)) {
          setPropertyValue(elements[i], propertyName, property[propertyName]);
        } else {
    ...
    for (let i = 0; i < elements.length; i++) {
      setPropertyValue(elements[i], property, String(value));
    }
  } else {
