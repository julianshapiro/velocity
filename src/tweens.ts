...
import {Normalizations} from "./normalizations/normalizations";
import {State} from "./state";
import {isFunction, isNumber, isNumberWhenParsed, isString} from "./types";
...
import {RegEx} from "./css/regex";
import {getUnit} from "./css/getUnit";
import {validateEasing} from "./validate";
import {TWEEN_NUMBER_REGEX} from "./constants";
...
commands.set("undefined", function (value, element, elements, elementArrayIndex, propertyName) {
  return fixColors(getPropertyValue(element, propertyName) || "");
});
 ...
 */
export function getUnitType(property: string): string {
...
export function expandProperties(animation: AnimationCall, properties: VelocityProperties) {
  ...
  for (const property in properties) {
    ...
    if (Array.isArray(valueData)) {
      ...
      if ((isString(arr1) && (/^[\d-]/.test(arr1) || RegEx.isHex.test(arr1))) || isFunction(arr1) || isNumber(arr1)) {
 ...
 */
export function explodeTween(propertyName: string, tween: VelocityTween, duration: number, isForcefeed?: boolean) {
  ...
  do {
    ...
    while (indexStart < startValue.length && indexEnd < endValue.length) {
      ...
      if (TWEEN_NUMBER_REGEX.test(charStart) && TWEEN_NUMBER_REGEX.test(charEnd)) {
        ...
        let unitStart = getUnit(startValue, indexStart), // temporary unit type
          unitEnd = getUnit(endValue, indexEnd); // temporary unit type
    ...
    if (!runAgain) {
      ...
      if (propertyName === "display") {
        ...
        && easing !== Easings["at-Start"] && easing !== Easings["during"] && easing !== Easings["at-end"]) {
  ...
  for (const propertyName in tweens) {
    ...
    if (tween[Tween.START] == null) {
      ...
      if (isString(startValue)) {
        tween[Tween.START] = fixColors(startValue) as any;
