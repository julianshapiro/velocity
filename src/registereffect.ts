...
export function animateParentHeight(elements: HTMLorSVGElement | HTMLorSVGElement[], direction, totalDuration, stagger) {
  ...
  ((elements as HTMLorSVGElement).nodeType ? [elements as HTMLorSVGElement] : elements as HTMLorSVGElement[]).forEach(function (element: HTMLorSVGElement, i) {
    ...
    if (getPropertyValue(element, "boxSizing").toString().toLowerCase() === "border-box") {
      propertiesToSum = ["height"];
    }

    propertiesToSum.forEach(function (property) {
      totalHeightDelta += parseFloat(getPropertyValue(element, property) as string);
    });
  });
          ...
          opts.begin = function () {
            ...
            if ((direction && direction[0] === "In") && propertyMap.opacity !== undefined) {
              (elements.nodeType ? [elements] : elements).forEach(function (element) {
                setPropertyValue(element, "opacity", 0);
              });
            }
