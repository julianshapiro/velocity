...
export function springAccelerationForState(state: springState) {
  return (-state.tension * state.x) - (state.friction * state.v);
}

export function springEvaluateStateWithDerivative(initialState: springState, dt: number, derivative: springDelta): springDelta {
  const state = {
...
export function springIntegrateState(state: springState, dt: number) {
  const a = {
      dx: state.v,
      dv: springAccelerationForState(state)
    },
