 ...
 * benefit from JIT compiling while still having a try/catch block.
 */
export function callProgress(activeCall: AnimationCall, timeCurrent: number) {
  try {
    const elements = activeCall.elements,
 ...
 * emulates it.
 */
let ticker: (callback: FrameRequestCallback) => number | Timer = document.hidden ? rAFProxy : rAFShim;
/**
